...
- `data/`: datos de muestra (anónimos) en formato CSV.
- `notebooks/`: análisis exploratorio y desarrollo del modelo.
- `scripts/`: scripts reutilizables para ETL, entrenamiento y segmentación.
...
## Project Structure
- `data/`: Sample or synthetic data (anonymized).
- `notebooks/`: Jupyter notebooks with exploratory analysis and model development.
- `scripts/`: Reusable Python scripts for ETL, model training, and segmentation.
  - `etl.py`: Data loading and cleaning.
  - `train_model.py`: Model training and evaluation.
  - `recovery_clients.py`: **Script to extract clients with 25–366 days without purchase** (used for recovery campaigns).
...
### 2. Customer Recovery Script (`recovery_clients.py`)
